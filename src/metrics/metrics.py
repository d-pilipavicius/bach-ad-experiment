...
def get_evaluator() -> Evaluator:
  return Evaluator(
    test_metrics=[
      ...
      F1AdaptiveThreshold(fields=["pred_score", "gt_label"], prefix="image_")
    ]
  )

def format_metrics(model_output: dict) -> str:
  metrics = "  TEST_DS  | AUROC_img | F1_img | AUPR_pixel | IoU_pixel | BEST_THOLD\n"
...
def _format_set(metrics: dict) -> str:
  return f"   {metrics['image_AUROC']:5.4f}  | {metrics['image_F1Score']:5.4f} |   {metrics['pixel_AUPR']:5.4f}   |  {metrics['pixel_IoU']:5.4f}   |   {metrics['image_F1AdaptiveThreshold']:5.4f}\n"
